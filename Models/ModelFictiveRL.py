...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 16 23:01:45 2022
...
class ModelFictiveRL(object):
    ...
    def learner(self, Q, respIdx, alpha, reward):
        """
        Args are the inputs to the model, besides the general model params:
        Args:
            Q: Vector of both Q values 
            respIdx: Indexes the chosen option
            alpha: delta-rule learning rate; scalar
            reward: observed reward outcome 
        """
        RPE = (reward - Q[respIdx])
        # Update chosen option
        Q[respIdx] =  Q[respIdx] + alpha * RPE
        # Update unchosen option 
        Q[1-respIdx] =  Q[1-respIdx] -  alpha * RPE        
        return(Q, RPE)
    ...
    def likelihood(self, param):
        ...
        for tI in np.arange(self.numTrials):
            ...
            if ~np.isnan(self.respIdx[tI]):
                ...
                qval, _ = self.learner(qval, respIdx, qA, reward)                                
            else:
                LL += 1e-10
        return -1*LL
    ...
    def simulate(self, param):
        ...
        for tI in np.arange(self.numTrials):
            ...
            if ~np.isnan(self.respIdx[tI]):
                ...
                reward = self.reward[tI]
                # Update learner 
                qval, RPE[tI] = self.learner(qval, respIdx, qA, reward)
