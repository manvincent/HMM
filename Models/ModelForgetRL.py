...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 16 22:46:55 2022
...
class ModelForgetRL(object):
    ...
    def __init__(self):
        ...
        self.param_bounds = dict2class(dict(alpha=(0.01,0.99),
                                            beta=(1,20),
                                            eta=(0.01,0.99)# min, max
                                            ))        
        return
    
    def forget_trial(self, Q, eta):
        Q = eta * Q + (1 - eta) * 0.5
        return Q
    ...
    def inputData(self, taskData):
        ...
        self.numTrials = self.numLLTrials = taskData.numTrials         
        self.respIdx = taskData.respIdx
        self.reward = taskData.reward
        self.abort = taskData.isAbort.astype(bool)
        return
        
    def likelihood(self, param):
        # Transform optimizer parameters
        # qA, smB = param
        qA, smB, eta = self.transformParams(param)
        # Initialize Q values
        ...
        for tI in np.arange(self.numTrials):
            ...
            # If incomplete trial            
            if self.abort[tI]:
                qval = self.forget_trial(qval, eta)            
    ...
    def transformParams(self, params):
        ...
        transParams[2] = max(self.param_bounds.eta) / (1 + np.exp(-1 * transParams[1])) # smB                
        return transParams
